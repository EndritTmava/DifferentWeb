...
namespace DifferentWeb.Models
{
    public class Student:Person
    {
        ...
        [RegularExpression("[A-Za-z]",ErrorMessage ="Parent name is not in the correct format, Parent Name should have only letters")]
        ...
        [RegularExpression(@" ^ ([0 - 9a - zA - Z]([\+\-_\.][0 - 9a - zA - Z] +) *)+@(([0-9a-zA-Z][-\\w]*[0-9a-zA-Z]*\\.)+[a-zA-Z0-9]{2,17})$", ErrorMessage = "Parent email is not in the correct format")]
        ...
        [RegularExpression(@"^\+?\d.\s?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{3}?", ErrorMessage = "Password is not in the correct format")]
